...
export async function exportAllData(): Promise<{
  ...
  const [sermonNotes, prayerPoints, actionSteps] = await Promise.all([
    db.sermonNotes.toArray(),
    db.prayerPoints.toArray(),
    db.actionSteps.toArray(),
  ])
  return { version: 1, exportedAt: new Date().toISOString(), sermonNotes, prayerPoints, actionSteps }
}
...
export async function repairData(): Promise<{ removed: number; synced: number }> {
  ...
  return { removed: orphanedPrayerIds.length + orphanedStepIds.length, synced }
}
